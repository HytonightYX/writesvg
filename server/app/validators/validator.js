...
class SearchValidator extends LinValidator {
  constructor() {
    super();
    // keyword 要查询的关键字
    this.q = [new Rule('isLength', '搜索关键字q不能为空', { min: 1, max: 16 })];
    // 分页:
...
class AddNoteValidator extends LinValidator {
  constructor() {
    super();
    this.title = [
      new Rule('isLength', '文章标题须在1-32个字符之间', { min: 1, max: 56 }),
    ];
    // this.tag = [
...
class NoteUpdateValidator extends PositiveIntegerValidator {
  constructor() {
    ...
    this.tag = [new Rule('isLength', '需要TagsID序列', { min: 1, max: 50 })];
  }
}
